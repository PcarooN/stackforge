  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const { data, error: authError } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            full_name: name, // SQL Trigger fonksiyonumuz bu datayı alıp profiles tablosuna yazar
          },
        },
      });
      ...
      // EĞER SUPABASE DASHBOARD'DA EMAIL CONFIRMATION AÇIKSA:
      // (Kullanıcının mail kutusuna doğrulama linki gider)
      if (data?.session === null) {
        alert("Kayıt başarılı! Lütfen e-posta adresinize gelen doğrulama linkine tıklayın.");
        window.location.href = "/login";
      } 
      // EĞER EMAIL CONFIRMATION KAPALIYSA:
      // (Kullanıcı otomatik giriş yapar ve session oluşur)
      else if (data?.user) {
        alert("Account provisioned successfully! Redirecting...");
        window.location.href = "/"; // Direkt ana sayfaya giriş yapmış olarak şutla
      }

    } catch (err: any) {
      if (err.message === "User already registered") {
        setError("Bu e-posta adresiyle daha önce kayıt olunmuş.");
      } else {
        setError(err.message || "Kayıt işlemi esnasında bir hata oluştu.");
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="space-y-1">
        <h1 className="text-2xl font-semibold text-white tracking-tight">Create cluster identity</h1>
        <p className="text-xs text-neutral-400 font-light">Register an operator profile to claim and provision software licenses.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-1.5">
          <label className="text-[10px] font-mono text-neutral-400 tracking-wider uppercase">OPERATOR_NAME</label>
          <div className="relative group">
            <User className="w-3.5 h-3.5 text-neutral-500 absolute left-3.5 top-1/2 -translate-y-1/2 group-focus-within:text-indigo-400 transition-colors" />
            <input
              type="text"
              value={name}
        ...
        >
          {loading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <>PROVISION_ACCOUNT <ArrowRight className="w-3.5 h-3.5" /></>}
        </button>
      </form>
      ...
      <div className="text-center pt-2">
        <p className="text-xs text-neutral-400 font-light">
          Already registered?{" "}
          <LinkComponent href="/login" className="text-indigo-400 hover:text-indigo-300 font-medium underline underline-offset-4 transition-colors">
            Sign in to console
          </LinkComponent>
        </p>
      </div>
